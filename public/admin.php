...
async function apiFetch(url, options = {}) {
    ...
    if (res.status === 401) { window.location.href = 'index.php'; throw new Error('No autorizado'); }
...
function showTab(tab) {
    ...
    const labels = { aulas: 'Gestión de Aulas', pcs: 'Gestión de Computadoras', usuarios: 'Gestión de Usuarios', auditoria: 'Registro de Auditoría' };
    const createLabels = { aulas: 'Nueva Aula', pcs: 'Nueva PC', usuarios: 'Nuevo Usuario', auditoria: '' };
...
function openCreateModal() { openModal(currentTab, 'create'); }
function openEditModal(entity, data) { openModal(entity, 'edit', data); }

function openModal(entity, mode, data = {}) {
    modalContext = { entity, mode, data };
    const titles = {
        aulas:    { create: 'Nueva Aula',    edit: 'Editar Aula'    },
        pcs:      { create: 'Nueva PC',      edit: 'Editar PC'      },
        usuarios: { create: 'Nuevo Usuario', edit: 'Editar Usuario' },
    };
    document.getElementById('modalTitle').textContent = titles[entity][mode];
    ...
    if (entity === 'pcs') card.classList.remove('max-w-2xl'), card.classList.add('max-w-3xl');
    ...
    if (entity === 'aulas') {
        fields.innerHTML =
            ...
            (mode === 'edit' ? selectField('Estado', 'activa', [{value:1,label:'Activa'},{value:0,label:'Inactiva'}], data.activa) : '');
    } else if (entity === 'pcs') {
        const aulaOpts = aulas.filter(a => Number(a.activa)).map(a => ({ value: a.id, label: `${a.id} - ${a.nombre}` }));
        ...
        fields.innerHTML =
            ...
            (mode === 'edit' ? selectField('Estado', 'activo', [{value:1,label:'Activo'},{value:0,label:'Inactivo'}], data.activo) : '');
    } else if (entity === 'aula_info') {
        const passwords = (data.passwords && typeof data.passwords === 'object') ? data.passwords : {};
        const sw = (data.software && typeof data.software === 'object') ? data.software : { instalados: [], no_instalados: [] };
        ...
        setTimeout(() => {
            ...
            else entries.forEach(([k, v]) => addPasswordRow({ cuenta: k, valor: v ?? '' }));
        }, 0);
    }
...
async function openAulaInfoModal(aulaId, aulaNombre) {
    ...
    openModal('aula_info', 'edit', { ...info, id_aula: aulaId, aula_nombre: aulaNombre });
...
document.getElementById('modalForm').addEventListener('submit', async function(e) {
    ...
    const { entity, mode, data } = modalContext;
    ...
    if (entity === 'aula_info') {
        ...
        const body = {
            ...
            software:     (instalados.length || noInstalados.length)
                            ? { instalados, no_instalados: noInstalados }
                            : null,
        };
        try {
    ...
    const endpoints = { aulas: '../backend/aulas.php', pcs: '../backend/computers.php', usuarios: '../backend/usuarios.php' };
...
async function confirmDelete(entity, id) {
    const labels = { aulas: 'desactivar esta aula', pcs: 'eliminar esta PC', usuarios: 'desactivar este usuario' };
...
// --- AUDITORÍA ---
let auditState = { page: 1, limit: 50, filtros: {} };
